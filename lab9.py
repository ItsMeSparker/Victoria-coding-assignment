...
POPULATION = "/Users/natipumvitayapradit/Downloads/populations.csv"
POPULATION_SMALL = "/Users/natipumvitayapradit/Downloads/populations_small.csv"
NUMBERS = "/Users/natipumvitayapradit/Downloads/numbers.txt"
NUMBERS_SMALL = "/Users/natipumvitayapradit/Downloads/numbers_small.txt"
EMPTY = "/Users/natipumvitayapradit/text_file.txt"
...
def count_evens_odds(fileName: str) -> dict[str, int]:
    ...
    >>> count_evens_odds(EMPTY)
    {'Even': 0, 'Odd': 0}
    >>> count_evens_odds(NUMBERS_SMALL)
    {'Even': 6, 'Odd': 8}
    >>> count_evens_odds(NUMBERS)
    {'Even': 588, 'Odd': 565}
    """
...
def get_number_frequencies(fileName: str) -> dict[int, int]:
    ...
    >>> get_number_frequencies(EMPTY)
    {}

    >>> get_number_frequencies(NUMBERS_SMALL)
    {865: 2, 771: 5, 633: 1, 200: 5, 866: 1}

    >>> get_number_frequencies(NUMBERS)
...
def most_freq_number(fileName: str) -> list[int]:
    ...
    >>> most_freq_number(EMPTY)
    []
    >>> most_freq_number(NUMBERS_SMALL)
    [200, 771]
    >>> most_freq_number(NUMBERS)
    [455, 927]
    """
    Dict = get_number_frequencies(fileName)
    Highest = 0
...
def create_population_dictionaries(fileName: str, granularity: int
    ...
    granularity > 0
    >>> create_population_dictionaries(EMPTY, 10000)
    ({}, {})
    >>> create_population_dictionaries(POPULATION_SMALL, 10000)
...
1: ['Nauru'], \
2: ['Palau']})
    >>> create_population_dictionaries(POPULATION_SMALL, 1000)
    ({'St. Martin (French part)': 31949, 'Nauru': 13049, 'Palau': 21503, \
'British Virgin Islands': 30661, 'San Marino': 33203, 'Gibraltar': 34408, \
'Monaco': 38499, 'Turks and Caicos Islands': 34900, 'Liechtenstein': 37666}, \
...
37: ['Liechtenstein']})
    """
    FileHandler = open(fileName, 'r')
    Line_List = FileHandler.readline()
    
    firstDict = {}
    secondDict = {}
        
    while Line_List:
        LineList = Line_List.split(",")
        firstDict[LineList[0]] = int(LineList[1][:-1])
        Popupation = int(LineList[1]) // granularity
        if Popupation in secondDict:
            secondDict[Popupation].append(LineList[0])
        else: secondDict[Popupation] = [LineList[0]]
        Line_List = FileHandler.readline()
    FileHandler.close()
...
def population_analysis(fileName: str, country: str, granularity: int
    ...
    granularity > 0
    >>> population_analysis(EMPTY, 'Canada', 1000)
    []
    >>> population_analysis(POPULATION_SMALL, 'Canada', 1000)
    []
    >>> population_analysis(POPULATION_SMALL, 'monaco', 1000)
    []
    >>> population_analysis(POPULATION_SMALL, 'San Marino', 1000)
    [(33203, 'San Marino')]
    >>> population_analysis(POPULATION_SMALL, 'Monaco', 10000)
...
(38499, 'Monaco')]
    >>> population_analysis(POPULATION, 'Canada', 1000000)
    [(36286425, 'Canada')]
    >>> population_analysis(POPULATION, 'Jamaica', 1000000)
...
(2876101, 'Albania'), (2881355, 'Jamaica'), (2924816, 'Armenia')]
    """
    result = []
    CountryDictOne, CountryDictTwo = create_population_dictionaries\
        (fileName, granularity)
    
    if country not in CountryDictOne:
            return result
        
    PopulationBinInterest = CountryDictOne[country] // granularity
 
    for CountryName in CountryDictTwo[PopulationBinInterest]:
        population = CountryDictOne[CountryName]
        result.append((population, CountryName))
